...
- **Demo App Downloads**: Pull full sample projects from the private `trustarc/ccm-mobile-consent-test-apps` repository (`release` by default on a plain `install.sh` run, or a ref explicitly selected via `APP_VERSION`, `TRUSTARC_REF`, or `REPO_REF`)
